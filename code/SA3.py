...
data = pd.read_csv(r"C:\Users\Anuraag  Kansara\Desktop\Critical Infrastructure\pasad-master\pasad-master\data\5 - Scenario SA3\xmeas10_001_data_1.csv")
print(data.shape)
train = data.iloc[:500,:].values
validation =  data.iloc[251:4000,:].values
test = data.iloc[3751:,:].values

oneSensor_train = train[:,8:9]
L=250
k=250
# Departure score
depScore=[]
...
# plotting 
# plt.plot([i for i in range(1000,4800)],depScore,'r-',label = 'Departure  Score')
# plt.hlines(maxVal,0,4800,colors='k', linestyles='dashed',label='alarm threshold')
...
plt.plot([i/100 for i in range(500)],train[:,8],'b',label='Training data')
plt.plot([i/100 for i in range(251,4000)],validation[:,8],'g',label ='Validation data')
plt.plot([i/100 for i in range(3751,4800)],test[:,8],'r',label='Testing data')
