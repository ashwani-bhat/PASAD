...
data = pd.read_csv(r"C:\Users\Anuraag  Kansara\Desktop\Critical Infrastructure\pasad-master\pasad-master\data\3 - Scenario SA1\xmv9_hundred_data_1.csv")
print(data.shape)
train = data.iloc[:500,:].values
validation =  data.iloc[251:4000,:].values
test = data.iloc[3751:,:].values

oneSensor_train = train[:,4:5]
L=250
k=250
# Departure score
depScore=[]
...
# plt.plot([i/100 for i in range(500)],train[:,4],'b',label='Training data')
# plt.plot([i/100 for i in range(251,4000)],validation[:,4],'g',label ='Validation data')
# plt.plot([i/100 for i in range(3751,4800)],test[:,4],'r',label='Testing data')
